...
public class UserFeatures {
    ...
    public String addUserToFile(String name, String mobileNumber, String userName, String password, String type) {
        ...
        if (type.equals("Customer")) {
            try {
                BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("C" + randomNumber + ".txt"));
                bufferedWriter.close();
            } catch (IOException e) {
                JOptionPane.showMessageDialog(null, "File Not Found");
            }
    ...
    public void addOrderToFile(String fileName, Order order) {
        try {
            ...
            bufferedWriter = new BufferedWriter(new FileWriter("order.txt", true));
    ...
    public void setFoodDataToFile(String food, String price) {

        try (FileWriter fw = new FileWriter("foods.txt", true);
             BufferedWriter bw = new BufferedWriter(fw);
             PrintWriter out = new PrintWriter(bw)) {
            out.println(food + ",RM" + price);
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "File not Found!");
        }
    }
    ...
    public void newFoodList(List food) {
        try (FileWriter fw = new FileWriter("foods.txt", false);
             BufferedWriter bw = new BufferedWriter(fw);
             PrintWriter out = new PrintWriter(bw)) {
            for (int i = 0; i < food.size(); i++) {
                out.println(food.get(i));
            }
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "File not Found!");
        }
    ...
    public String salesReport() {
        ...
        output.append("\n----------------------------------------------Sales Report----------------------------------------------\n").append("\n");
        output.append("  " + "Foods" + "\t\t" + "Quantity" + "\t" + "Price per Item" + "\t" + "Total Revenue" + "\n\n");
        ...
        try {
            ...
            while ((foodLine = bufferedReader.readLine()) != null) {
                food = foodLine.split(",")[0];
                price = foodLine.split(",")[1];

                int i = 0;
                String order_line;
                BufferedReader test = new BufferedReader(new FileReader("order.txt"));
                while ((order_line = test.readLine()) != null) {
                    i += order_line.split(food, -1).length - 1;
                }
                BigDecimal value = new BigDecimal(price.substring(2)).setScale(2, RoundingMode.HALF_UP);
                output.append("  " + food + "\t\t" + i + "\t" + price + "\tRM" + value.multiply(new BigDecimal(i)) + "\n");
                total = total.add(value.multiply(new BigDecimal(i)));
                test.close();
            }
            bufferedReader.close();
        }
